...
let hmrEnabled = !!(globalThis.module as any)?.hot
let updateTimeout = 0
...
function identifyRegistration<R extends Registration>(registrations: R[], name: string, sourceFrameOffset: number): [id: string | undefined, index: number] {
    ...
    let callerModule = stack[sourceFrameOffset]?.split(new RegExp('(?<!\\/)@|\\('))[0] ?? '<unknown>' // Strip off line number - module or function level is good enough
